...
(
    ...
    -- Data autorizzaizone prefettura
    ...
    CONSTRAINT odo_pkey PRIMARY KEY (s_id),
    CONSTRAINT odo_operazione_fkey FOREIGN KEY (operazione)
        REFERENCES accessi.operations_sinc_anncsu (op_ty) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT odo_localita_fkey FOREIGN KEY (odo_loc)
        REFERENCES stradario.localita_list (localita_name) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT odo_dug_fkey FOREIGN KEY (odo_dug)
        REFERENCES stradario.dug_list (dug_name) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT odo_flag_delibera_fkey FOREIGN KEY (odo_flag_delibera)
        REFERENCES stradario.flag_delibera_list (flag_delibera_ty) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT odo_aut_prefettura_coerenza_check CHECK (aut_prefettura_data IS NULL AND protocollo_pref IS NULL OR aut_prefettura_data IS NOT NULL AND protocollo_pref IS NOT NULL),
    CONSTRAINT odo_flag_delibera_check CHECK (flag_delibera IS NULL OR (flag_delibera = ANY (ARRAY['0'::bpchar, '1'::bpchar, '2'::bpchar, '3'::bpchar, '4'::bpchar]))),
    CONSTRAINT odo_progr_nazionale_coerenza_check CHECK (allineato = true OR operazione IS NULL OR operazione::text = 'I'::text AND progressivo_nazionale IS NULL OR (operazione::text = ANY (ARRAY['R'::character varying, 'S'::character varying]::text[])) AND progressivo_nazionale IS NOT NULL),
    CONSTRAINT odo_provvedimento_coerenza_check CHECK (allineato = true OR flag_delibera IS NULL OR (flag_delibera = ANY (ARRAY['0'::bpchar, '1'::bpchar])) AND provvedimento_data IS NOT NULL AND provvedimento_protocollo IS NOT NULL OR (flag_delibera = ANY (ARRAY['2'::bpchar, '3'::bpchar, '4'::bpchar])) AND provvedimento_data IS NULL AND provvedimento_protocollo IS NULL)
);
